...
const STATS := {
	Kind.TANK: {"max_hp": 150, "damage": 8, "speed": 52.0, "attack_range": 44.0, "cooldown": 1.05, "projectile": false, "heal": 0},
	Kind.MELEE: {"max_hp": 82, "damage": 14, "speed": 82.0, "attack_range": 46.0, "cooldown": 0.8, "projectile": false, "heal": 0},
	Kind.RANGED: {"max_hp": 48, "damage": 10, "speed": 70.0, "attack_range": 168.0, "cooldown": 1.15, "projectile": true, "heal": 0},
	Kind.SUPPORT: {"max_hp": 58, "damage": 5, "speed": 68.0, "attack_range": 52.0, "cooldown": 1.1, "projectile": false, "heal": 11},
}
const COST := {Kind.TANK: 70, Kind.MELEE: 45, Kind.RANGED: 60, Kind.SUPPORT: 55}
const DISPLAY := {
...
const KERNEL_MULT := [[1.0, 0.5, 1.0, 1.0], [1.5, 1.0, 0.5, 1.5], [1.0, 1.5, 1.0, 1.0], [1.0, 0.5, 1.0, 1.0]]
...
func setup(p_team: Team, p_kind: Kind, pos: Vector2, p_tower: Node2D, p_game: Node = null) -> void:
	team = p_team
	kind = p_kind
	position = pos
	enemy_tower = p_tower
...
static func affinity_multiplier(attacker_kind: int, defender_kind: int) -> float:
	if attacker_kind < 0 or defender_kind < 0 or attacker_kind >= 4 or defender_kind >= 4:
		return 1.0
...
func _process(delta: float) -> void:
	...
	if target != null and _distance_to(target) <= attack_range:
		_try_attack(target)
		if uses_projectile and target.is_in_group("units") and _distance_to(target) < attack_range * 0.52:
...
func _notify_affinity(text: String) -> void:
	if game_ref != null and is_instance_valid(game_ref) and game_ref.has_method("_emit_toast"):
		game_ref._emit_toast(text, 3.5)
...
func _spawn_float_text(text: String) -> void:
	...
	lab.add_theme_color_override("font_color", Color("2e6b3a") if text.begins_with("SE") else Color("8b2e28"))
	lab.position = Vector2(-90, -70)
	lab.size = Vector2(220, 28)
	lab.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	add_child(lab)
	var tw := create_tween()
	tw.tween_property(lab, "position:y", lab.position.y - 42.0, 1.4)
	tw.parallel().tween_property(lab, "modulate:a", 0.0, 1.4)
...
func _try_attack(target: Node2D) -> void:
	...
	if hit_unit:
		...
		var atk_kind: int = int(kind)
		var mult: float = affinity_multiplier(atk_kind, def_kind)
		dmg = maxi(1, int(round(float(dmg) * mult)))
		if team == Team.PLAYER and _note_cd <= 0.0 and absf(mult - 1.0) > 0.05:
			var atk: String = str(DISPLAY[atk_kind]["affinity"])
...
func _build_visual() -> void:
	...
	shadow.polygon = PackedVector2Array([Vector2(-wide, 10), Vector2(wide, 10), Vector2(wide - 4, 18), Vector2(-wide + 4, 18)])
	add_child(shadow)
	move_child(shadow, 0)
	match kind:
		Kind.TANK: _build_cragback()
		Kind.MELEE: _build_knuckhorn()
		Kind.RANGED: _build_veilray()
		Kind.SUPPORT: _build_gleamlet()
...
func _pal() -> Dictionary:
	...
	match kind:
		Kind.TANK: type_accent = Color("c4a574")
		Kind.MELEE: type_accent = Color("e08a5a")
		Kind.RANGED: type_accent = Color("b48ae0")
		Kind.SUPPORT: type_accent = Color("f0a0c8")
	if team == Team.PLAYER:
